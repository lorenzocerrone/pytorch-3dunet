    ...
    def __call__(self, m):
        ...
        if m.ndim == 3:
            ...
            for c in range(m.shape[0]):
                ...
                channels.append(
                    np.pad(result, pad_width=((0, 0), y_pad, x_pad), mode="reflect")
                )
            return np.stack(channels, axis=0)
...
class BlobsToMask:
    ...
    def __init__(
        self, append_label=False, boundary=False, cross_entropy=False, **kwargs
    ):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert isinstance(offsets, list) or isinstance(
            offsets, tuple
        ), "offsets must be a list or a tuple"
        ...
        if z_offsets is not None:
            assert len(offsets) == len(
                z_offsets
            ), "z_offsets length must be the same as the length of offsets"
        else:
...
class AdditivePoissonNoise:
    def __init__(
        self, random_state, lam=(0.0, 1.0), execution_probability=0.1, **kwargs
    ):
        self.execution_probability = execution_probability
        self.random_state = random_state
        self.lam = lam
...
class ExpandandRepeat:
    """Expand the input tensor along the specified axis and repeat it `num_repeats` times."""
    ...
    def __call__(self, m):
        m = np.expand_dims(m, axis=self.axis)
        if self.num_repeats is None:
            return m

        repeat_dims = [1] * m.ndim
        repeat_dims[self.axis] = self.num_repeats
        return np.tile(m, repeat_dims)


class MaskCheck:
    """Ensure that the input tensor is a binary mask, and optionally inverts it."""
